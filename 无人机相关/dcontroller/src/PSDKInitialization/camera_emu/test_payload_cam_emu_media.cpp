...
#include "dji_media_file_manage/dji_media_file_core.h"
                                                      ...
                                                      uint16_t outPathBufferSize)
{
    ...
    directory = (char*)osalHandler->Malloc(DJI_FILE_PATH_SIZE_MAX);
    if (directory == NULL) {
                                    ...
                                    uint32_t frameInfoBufferCount, uint32_t *frameCount)
{
    ...
    frameInfoString = (char*)osalHandler->Malloc(VIDEO_FRAME_MAX_COUNT * 1024);
...
static T_DjiReturnCode DeleteMediaFile(char *filePath)
{
    ...
    // zbnote: the function 'DjiFile_Delete' is not defined in the sample application.
...
static void *UserCameraMedia_SendVideoTask(void *arg)
{
    ...
    videoFilePath = (char*)osalHandler->Malloc(DJI_FILE_PATH_SIZE_MAX);
    if (videoFilePath == NULL) {
        USER_LOG_ERROR("malloc memory for video file path fail.");
        exit(1);
    }

    transcodedFilePath = (char*)osalHandler->Malloc(DJI_FILE_PATH_SIZE_MAX);
    if (transcodedFilePath == NULL) {
        USER_LOG_ERROR("malloc memory for transcoded file path fail.");
        exit(1);
    }

    frameInfo = (T_TestPayloadCameraVideoFrameInfo *)osalHandler->Malloc(
                VIDEO_FRAME_MAX_COUNT * sizeof(T_TestPayloadCameraVideoFrameInfo));
    if (frameInfo == NULL) {
        USER_LOG_ERROR("malloc memory for frame info fail.");
        exit(1);
    }
    ...
    while (1) {
        ...
        send:
            ...
            dataBuffer = (char*)calloc(frameBufSize, 1);
            if (dataBuffer == NULL) {
