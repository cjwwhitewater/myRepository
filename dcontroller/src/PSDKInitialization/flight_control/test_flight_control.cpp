...
bool DjiTest_FlightControlCheckActionStarted(E_DjiFcSubscriptionDisplayMode mode)
{
    ...
    while (DjiTest_FlightControlGetValueOfDisplayMode() != mode && actionNotStarted
           < timeoutCycles) {
        actionNotStarted++;
        s_osalHandler->TaskSleepMs(100);
    }
    ...
    if (actionNotStarted == timeoutCycles) {
        USER_LOG_ERROR("%s start failed, now flight is in %s.",
            s_flightControlDisplayModeStr
                   [DjiTest_FlightControlGetDisplayModeIndex(mode)].displayModeStr,
            s_flightControlDisplayModeStr
                   [DjiTest_FlightControlGetDisplayModeIndex(
                       (E_DjiFcSubscriptionDisplayMode)DjiTest_FlightControlGetValueOfDisplayMode())].displayModeStr);
        return false;
    } else {
