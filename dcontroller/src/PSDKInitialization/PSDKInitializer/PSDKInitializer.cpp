...
PSDKInitializer::PSDKInitializer()
{
    PSDKInitializer::setupEnvironment();
    PSDKInitializer::startPSDK();
...
void PSDKInitializer::setupEnvironment()
{
    ...
    printConsole.func = printToConsole;
    ...
    localRecordConsole.func = localWrite;
    ...
    if (localWriteFsInit(DJI_LOG_PATH) !=
...
void PSDKInitializer::startPSDK()
{
    ...
    returnCode = fillInUserInfo(&userInfo);
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        throw runtime_error("error when filling user info.");
    }

    returnCode = DjiCore_Init(&userInfo);
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        throw runtime_error("error when initing psdk core.");
    }

    returnCode = DjiAircraftInfo_GetBaseInfo(&aircraftInfoBaseInfo);
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        throw runtime_error("error when getting aircraft base info.");
    }

    if (aircraftInfoBaseInfo.mountPosition != DJI_MOUNT_POSITION_EXTENSION_PORT) {
        throw runtime_error("Please run this application on E-port.");
    }

    returnCode = DjiCore_SetAlias("PSDK_APPALIAS");
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        throw runtime_error("error when setting alias.");
    }

    returnCode = DjiCore_SetFirmwareVersion(firmwareVersion);
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        throw runtime_error("error when setting firmware version.");
    }

    returnCode = DjiCore_SetSerialNumber("PSDK12345678XX");
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        throw runtime_error("error when setting serial number");
    }

    returnCode = DjiCore_ApplicationStart();
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        throw std::runtime_error("error when starting pskk core");
    }
    cout << "PSDK started.";
}

T_DjiReturnCode PSDKInitializer::printToConsole(const uint8_t *data,
                                                uint16_t dataLen)
{
    printf("%s", data);
...
T_DjiReturnCode PSDKInitializer::localWrite(const uint8_t *data, uint16_t dataLen)
{
    int32_t realLen;
...
T_DjiReturnCode PSDKInitializer::fillInUserInfo(T_DjiUserInfo *userInfo)
{
    ...
    if (strlen(USER_APP_NAME) >= sizeof(userInfo->appName) ||
        ...
        USER_LOG_ERROR("Length of user information string is beyond limit.");
    ...
    if (!strcmp(USER_APP_NAME, "your_app_name") ||
        ...
        USER_LOG_ERROR("Please fill in correct user information.");
        return DJI_ERROR_SYSTEM_MODULE_CODE_INVALID_PARAMETER;
    }
...
T_DjiReturnCode PSDKInitializer::localWriteFsInit(const char *path)
{
    ...
    if (s_djiLogFileCnt == nullptr) {
        ...
        ret = fread((uint16_t * ) & logFileIndex, 1, sizeof(uint16_t),
                    s_djiLogFileCnt);
