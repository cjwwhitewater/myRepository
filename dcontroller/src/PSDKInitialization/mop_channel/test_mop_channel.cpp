...
static void *DjiTest_MopChannelSendNormalTask(void *arg)
{
    ...
    sendBuf = (uint8_t*)osalHandler->Malloc(TEST_MOP_CHANNEL_NORMAL_TRANSFOR_SEND_BUFFER);
    if (sendBuf == NULL) {
...
REACCEPT:
    ...
    recvBuf = (uint8_t*)osalHandler->Malloc(TEST_MOP_CHANNEL_NORMAL_TRANSFOR_RECV_BUFFER);
    if (recvBuf == NULL) {
...
static void *DjiTest_MopChannelFileServiceSendTask(void *arg)
{
    ...
    sendBuf = (uint8_t*)osalHandler->Malloc(TEST_MOP_CHANNEL_FILE_SERVICE_SEND_BUFFER);
    if (sendBuf == NULL) {
...
static void *DjiTest_MopChannelFileServiceRecvTask(void *arg)
{
    ...
    recvBuf = (uint8_t*)osalHandler->Malloc(TEST_MOP_CHANNEL_FILE_SERVICE_RECV_BUFFER);
    if (recvBuf == NULL) {
